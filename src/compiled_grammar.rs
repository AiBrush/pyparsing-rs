...
pub enum CompiledPattern {
    Literal { bytes: Vec<u8>, first: u8 },
    Word { init_table: [bool; 256], body_table: [bool; 256] },
    Regex { pattern: regex::Regex },
    Sequence { patterns: Vec<CompiledPattern> },
    Choice { patterns: Vec<CompiledPattern> },
}
...
impl CompiledPattern {
    pub fn match_input(&self, input: &str) -> Option<usize> {
        ...
        match self {
            ...
            CompiledPattern::Word { init_table, body_table } => {
            ...
            CompiledPattern::Regex { pattern } => {
                pattern.find(input).map(|m| m.end())
            }
...
impl FastParser {
    ...
    fn word(init_chars: &str, body_chars: Option<&str>) -> Self {
        ...
        Self {
            pattern: CompiledPattern::Word { init_table, body_table },
        }
    }
...
#[pyfunction]
pub fn ultra_fast_literal_match(
    py: Python<'_>,
    inputs: Vec<String>,
    pattern: &str,
) -> usize {
